...
| M2 | Section entrance (gesture rows, how-it-works points, timings) on scroll | **Attention** — guides the eye down a long explanatory page, once | 520 ms, rise 16 px, stagger 60 ms, `--ease` | Observer never installed; every element is at its final state on paint. |
