...
**FIX round 3 (2026-09-19):** the reel `flex09x3` (a flex whose other hand the hand model reads as a perfect thumbs-up, hard-cut three times) now fires Goblin Muscle three times and Thumbs Up never, in the VIDEO-mode gate (flex at 600 / 700 / 700 ms from the cut) and through the real pipeline (local production build, Mac GPU: 18 of 18 passes over six runs at 1000 and 390 px, 366–620 ms; the round-2 build played Thumbs Up 5 of 6). Round 2 had widened that reel's ground truth to accept either emote; the `accept` mechanism is gone from the builder, the judge and the e2e harness, so every event is satisfied only by its labelled emote. Every other reel, the official clip, the throttled (~12 fps) and Playwright Chromium runs are unchanged: 9 of 9 gated reels, 70 of 70 clips, 100 % everywhere; clip latency median 160 ms (was 240: restarting the flex average when the pose lands also makes flexes after a cut faster), p95 500, max 700. Still open from the round-3 report: `yawn-17` does not fire on video, the 0.6 s-rest repeats merge (thumbs-up 2 of 3, flex 3 of 3, yawn 3 of 3 this round), the phone fold. What the engine still guarantees is one emote per hold, never two: a flex that emerges from a thumbs-up still being held takes the hold over silently.
